...
#:: Purpose:     Enable initial configuration of a newly built Linux system.
#::              Sets up auto-login, MOTD, and prepares for OEM configuration.
#:: Usage:       Execute as root. No parameters required.
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
readonly CONFIG_DIR="${BASE_DIR}/config" # Consistent with _CONFIG_DIR from original
readonly LOG_DIR_PATH="${BASE_DIR}/logs" # Consistent with _LOG_DIR from original

readonly LIBRARY_PATH="${CORE_DIR}/_rtd_library"
readonly FAILLOG_PATH="${BASE_DIR}/faillog.log" # Consistent with _faillog

# --- Load Core Library ---
if [[ ! -f "$LIBRARY_PATH" ]]; then
    echo "CRITICAL ERROR: Required library not found at '${LIBRARY_PATH}'." >&2
    # Attempt to log to faillog if possible, but prioritize stderr for immediate visibility
    echo "$(date '+%Y-%m-%d %H:%M:%S') CRITICAL ERROR: Library '${LIBRARY_PATH}' not found." >> "$FAILLOG_PATH" 2>/dev/null || true
    exit 1
fi
# shellcheck source=/dev/null # Tell shellcheck to ignore sourcing a variable path
source "$LIBRARY_PATH" || {
...
# Use a function for repeated mkdir -p and logging
ensure_directory_exists() {
    local dir_path="$1"
    if ! mkdir -p "$dir_path"; then
        system::log_item "ERROR: Failed to create directory '${dir_path}'." # Assumes system::log_item handles errors
        # Consider exiting if critical, or returning a failure code
        return 1
    fi
...
ensure_directory_exists "$_LOG_DIR" || exit 1 # Exit if log dir can't be made
ensure_directory_exists "$_CONFIG_DIR" || exit 1

# --- Wisdom Quotes Setup ---
readonly RTD_WISDOM_QUOTES_PATH="${_CONFIG_DIR}/kens_quotes.txt" # Use consistent naming
readonly KENS_QUOTES="
...
" # Consider moving this to a separate data file for cleanliness
...
configure_motd() {
    ...
    local dtype
    dtype=$(system::distribution_type) # Assumes this function returns a normalized type
                                      # e.g., "debian", "ubuntu", "fedora", "suse", "rhel"
